...
const Cocktails = ({items}) => {
  ...
  return (
    <Stack component={'section'} direction={'row'} flexWrap={'wrap'} gap={'50px'} margin={'0 auto 50px auto'} justifyContent={'center'}>
        {cocktails.length>0?
        items.map(item=>{
            return <Link className='link' key={item.idDrink} to={`/cocktails/${item.idDrink}`}><Stack width={'250px'}  height={'auto'} key={item.idDrink} sx={{backgroundColor: 'primary.main', borderRadius: '10%', overflow: 'hidden', boxShadow: '10px -5px 5px 0px rgba(0,0,0,0.7)',  
        }}>
